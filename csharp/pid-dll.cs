...
public class pidDLL
{
    ...
    // Struct (already static implicitly)
    ...
    private static void FatalError(string msg, int exitCode)
    {
        Console.Error.WriteLine($"[!] FATAL Error: {msg} Error Code: {Marshal.GetLastWin32Error()}");
        throw new InvalidOperationException($"Operation failed: {msg} (Win32 Error: {Marshal.GetLastWin32Error()})");
    }

    private static void WriteRemoteMemory(IntPtr procHandle, IntPtr addr, byte[] data, string errorMsg, Action cleanup)
    {
    ...
    private static void ReadRemoteMemory(IntPtr procHandle, IntPtr addr, byte[] buffer, string errorMsg, Action cleanup)
    {
    ...
    public static bool Blacklist(uint targetPid, int unhookDelay, string blacklistFilePath = "blacklist.txt")
    {
        try
        {
            ...
            List<string> blacklistWStringsStorage = ReadBlacklistFromFile(blacklistFilePath);
            if (blacklistWStringsStorage.Count == 0)
            {
                FatalError($"No DLL names found in {blacklistFilePath} or file could not be read. Exiting.", -50);
            }
            ...
            currentBlacklistPtrsValues.Add(0); // Null terminator for the array of pointers
        ...
        {
            Console.Error.WriteLine($"[!] An error occurred during injection: {ex.Message}");
            PerformCleanup(); 
            return false; 
        }
    }
