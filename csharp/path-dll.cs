...
public class pathDLL
{
    ...
    private const int OFF_BLACKLISTARRAY_DATA = 0x17C; // Not used in C# but present in original stub.
    private const int OFF_COMPAREWIDESTRINGS = 0x0;    // Not used in C# but present in original stub.
    private const int OFF_DATA_SIZE = 0x28;            // Not used in C# but present in original stub.
    ...
    private const int OFF_TEXT_MN_SIZE = 0x15C;        // This seems to be a duplicate of OFF_ADDRBLACKLISTSTR_PTR

    // Custom Exception class for more structured error handling
    public class LdrLoadDllHookerException : Exception
    {
        public int Win32ErrorCode { get; }

        public LdrLoadDllHookerException(string message, int win32ErrorCode) : base(message)
        {
            Win32ErrorCode = win32ErrorCode;
        }

        public LdrLoadDllHookerException(string message) : base(message)
        {
            Win32ErrorCode = 0; // No specific Win32 error
        }
    }

    // Helper to throw a custom exception
    private static void ThrowFatalError(string msg, int code)
    {
        throw new LdrLoadDllHookerException(msg, code);
    }
    ...
    private static List<string> ReadBlacklistFromFile(string filename)
    {
        ...
        {
            throw new LdrLoadDllHookerException($"Blacklist file not found: {filename}");
        }
        ...
        {
            throw new LdrLoadDllHookerException($"Error reading blacklist file '{filename}': {ex.Message}");
        }
        return blacklistStrings;
    }

    private static void WriteRemoteMemory(SafeProcessHandle procHandle, IntPtr addr, byte[] data, string errorMsg)
    {
        ...
        {
            ThrowFatalError(errorMsg, Marshal.GetLastWin32Error());
        }
    }

    private static void WriteRemoteMemory(SafeProcessHandle procHandle, IntPtr addr, IntPtr dataPtr, uint size, string errorMsg)
    {
        UIntPtr lpNumberOfBytesWritten;
    ...
    public static bool Blacklist(uint targetPid, string targetPath, int unhookDelaySeconds, string blacklistFilePath = "blacklist.txt")
    {
        bool createNewProcess = !string.IsNullOrEmpty(targetPath);

        if ((targetPid != 0 && createNewProcess) || (targetPid == 0 && !createNewProcess))
        {
            ThrowFatalError("Please specify either targetPid or targetPath, but not both.", -1);
        }
        if (unhookDelaySeconds < 0)
        {
            ThrowFatalError("Unhook delay must be a non-negative integer for seconds.", -2);
        }

        SafeProcessHandle procHandle = null;
        IntPtr hThread = IntPtr.Zero; // Only used if creating a new process
        ...
        try
        {
            if (createNewProcess)
            {
                ...
                Console.WriteLine($"[+] Creating process '{targetPath}' suspended in a new console window...");
                if (!NativeMethods.CreateProcess(null, targetPath, IntPtr.Zero, IntPtr.Zero, false,
                                                 NativeMethods.CREATE_SUSPENDED | NativeMethods.CREATE_NEW_CONSOLE,
                                                 IntPtr.Zero, null, ref si, out pi))
                {
                    ThrowFatalError("CreateProcess failed", Marshal.GetLastWin32Error());
                }

                procHandle = new SafeProcessHandle(pi.hProcess, true);
                targetPid = pi.dwProcessId;
                hThread = pi.hThread; // Store the thread handle for resuming
                Console.WriteLine($"[+] Process '{targetPath}' created (PID: {targetPid}, TID: {pi.dwThreadId}) in suspended state.");
            }
            else // Attaching to an existing process
            {
                ...
                {
                    ThrowFatalError("OpenProcess failed", Marshal.GetLastWin32Error());
                }
            ...
            if (hNtdll == IntPtr.Zero) ThrowFatalError("GetModuleHandle failed for ntdll.dll", Marshal.GetLastWin32Error());

            IntPtr addrLdrLoadDllLocal = NativeMethods.GetProcAddress(hNtdll, "LdrLoadDll");
            if (addrLdrLoadDllLocal == IntPtr.Zero) ThrowFatalError("GetProcAddress failed for LdrLoadDll", Marshal.GetLastWin32Error());
            ...
            {
                ThrowFatalError("Failed to read original LdrLoadDll prologue", Marshal.GetLastWin32Error());
            }
            ...
            List<string> blacklistStringsStorage = ReadBlacklistFromFile(blacklistFilePath);
            if (blacklistStringsStorage.Count == 0)
            {
                ThrowFatalError($"No DLL names found in '{blacklistFilePath}' or file could not be read. Exiting.", -50);
            }
            ...
            {
                remoteBlacklistPtrsToWrite.Add(IntPtr.Zero); // Placeholder
                remoteBlacklistStringsSize += (uint)((ws.Length + 1) * sizeof(char));
                remoteBlacklistStringsSize = (remoteBlacklistStringsSize + 7) & ~7U; // Align to 8 bytes
            }
            remoteBlacklistPtrsToWrite.Add(IntPtr.Zero); // Null terminator for the array of pointers
                ...
                procHandle))
            {
                ...
                {
                    ThrowFatalError("VirtualAllocEx failed", Marshal.GetLastWin32Error());
                }
                ...
                {
                    ThrowFatalError("Failed to change remote stub memory protection to RX", Marshal.GetLastWin32Error());
                }
                ...
                {
                    ThrowFatalError("VirtualProtectEx failed (pre-hook)", Marshal.GetLastWin32Error());
                }
                ...
                if (!NativeMethods.WriteProcessMemory(procHandle, addrLdrLoadDllLocal, hook, (UIntPtr)12, out lpNumberOfBytesWrittenForHook))
                {
                    ...
                    ThrowFatalError("Failed to write hook to LdrLoadDll", Marshal.GetLastWin32Error());
                }
                ...
                if (createNewProcess)
                {
                    Console.WriteLine($"[+] Resuming {targetPath} (PID {targetPid}, TID: {NativeMethods.GetThreadId(hThread)})...");
                    if (NativeMethods.ResumeThread(hThread) == NativeMethods.INVALID_HANDLE_VALUE.ToInt64())
                    {
                        ThrowFatalError("ResumeThread failed", Marshal.GetLastWin32Error());
                    }
                    Console.WriteLine($"[+] Successfully resumed {targetPath}.");
                }
                else
                {
                ...
                if (unhookDelaySeconds > 0)
                {
                    Console.WriteLine($"[+] Unhooking in {unhookDelaySeconds} seconds...");
                    Thread.Sleep(unhookDelaySeconds * 1000);
                ...
                {
                    Console.WriteLine("[+] Unhook delay not specified or zero. Hook will remain active until process terminates or is manually unhooked.");
                }
            }
            return true;
        }
        catch (LdrLoadDllHookerException ex)
        {
            Console.Error.WriteLine($"[!] Hooking failed: {ex.Message} (Win32 Error: {ex.Win32ErrorCode})");
            return false;
        }
        catch (Exception ex)
        {
